...
func main() {
	...
	var enableLocalClusterImport bool
	var localClusterManagedClusterName string
	var localClusterNamespace string
	flag.StringVar(&metricsAddr, "metrics-bind-address", ":8080", "The address the metric endpoint binds to.")
	flag.StringVar(&probeAddr, "health-probe-bind-address", ":8081", "The address the probe endpoint binds to.")
	flag.BoolVar(&enableLeaderElection, "leader-elect", false,
		"Enable leader election for controller manager. "+
			"Enabling this will ensure there is only one active controller manager.")
	flag.BoolVar(&enableLocalClusterImport, "enable-local-cluster-import", true, "Should local cluster import be enabled? (default:true)")
	flag.StringVar(&localClusterManagedClusterName, "local-managed-cluster-name", "local-cluster", "The name that is used for the ManagedCluster created for the local-cluster.")
	flag.StringVar(&localClusterNamespace, "local-managed-cluster-namespace", "local-cluster", "The namespace to be used for local-cluster related CRs")
	opts := zap.Options{}
	...
	if enableLocalClusterImport {
		log.Info("Local cluster import is enabled in the operator, creating LocalClusterImportReconciler")
		if err = (controllers.NewLocalClusterImportReconciler(mgr.GetClient(), "local-cluster", "agent", log)).SetupWithManager(mgr); err != nil {
