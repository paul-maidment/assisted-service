...
var _ = Describe("Bootstrap Ignition Update", func() {
	...
	var (
		...
		installerCache *installercache.MockInstallerCache
	)
	...
	BeforeEach(func() {
		...
		installerCache = installercache.NewMockInstallerCache(ctrl)
		...
		g := NewGenerator(workDir, cluster, "", "", "", "", mockS3Client, logrus.New(), nil, "", "", manifestsAPI, eventsHandler, installerCache, 100, 1*time.Second).(*installerGenerator)
	...
	Context("Handle installercache retry", func() {
		...
		BeforeEach(func() {
			releaseRetryMax := 3
			releaseRetryInterval := 10 * time.Millisecond
			generator = NewGenerator(workDir, cluster, "", "", "", "", mockS3Client, logrus.New(), nil, "4.18.7", "", manifestsAPI, eventsHandler, installerCache, releaseRetryMax, releaseRetryInterval).(*installerGenerator)
		})

		It("should be able to retrieve a release using the installer cache, succesful first try", func() {
			eventsHandler.EXPECT().V2AddMetricsEvent(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(1)
			installerCache.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(
				installercache.NewRelease(eventsHandler, strfmt.UUID(uuid.NewString()), "4.18.7", time.Now()),
				nil,
			).Times(1)
			err = generator.Generate(context.TODO(), []byte{})
			// We don't actually want to test the Generate function beyond the point of grabbing the release.
			// So expect the first failure after this to prove we have moved past the fetch.
			Expect(err.Error()).To(ContainSubstring("Malformed version:"))
		})

		It("should retry get from installer cache if the call fails with timeout", func() {
			eventsHandler.EXPECT().V2AddMetricsEvent(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(1)
			// First attempt should fail
			// TODO: Make this an error type specific to timeout so that we can selectively ignore it while allowing other errors to quickly propagate.
			installerCache.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(
				nil,
				&installercache.ErrorInsufficientCacheCapacity{Message: "timed out while attempting to fetch a release"},
			).Times(1)
		...
		It("should not retry if error from installer cache is not related to storage capacity", func() {
			installerCache.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(
				nil,
				fmt.Errorf("Some random error"),
			).Times(1)
			err = generator.Generate(context.TODO(), []byte{})
			Expect(err.Error()).To(ContainSubstring("failed to get installer path for release 4.18.7"))
		})

		It("should time out retries after max attempts made", func() {
			installerCache.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(
				nil,
				&installercache.ErrorInsufficientCacheCapacity{Message: "timed out while attempting to fetch a release"},
			).Times(4)
			err = generator.Generate(context.TODO(), []byte{})
			Expect(err.Error()).To(BeEquivalentTo("failed to get installer path after 3 retries for release 4.18.7: timed out while attempting to fetch a release"))
		})
...
-----END CERTIFICATE-----`
	...
	BeforeEach(func() {
		...
		manifestsAPI = manifestsapi.NewMockManifestsAPI(ctrl)
		eventsHandler = eventsapi.NewMockHandler(ctrl)
		installerCache, err = installercache.New(filepath.Join(workDir, "install-config-generate", "installercache"), 5, 5, eventsHandler, logrus.New(), metrics.NewOSDiskStatsHelper())
		Expect(err).NotTo(HaveOccurred())
	})
	...
	Describe("update ignitions", func() {
		It("with ca cert file", func() {
			g := NewGenerator(workDir, cluster, "", "", caCertPath, "", nil, logrus.New(), nil, "", "", manifestsAPI, eventsHandler, installerCache, 100, 1*time.Second).(*installerGenerator)
		...
		It("with no ca cert file", func() {
			g := NewGenerator(workDir, cluster, "", "", "", "", nil, logrus.New(), nil, "", "", manifestsAPI, eventsHandler, installerCache, 100, 1*time.Second).(*installerGenerator)
...
var _ = Describe("Import Cluster TLS Certs for ephemeral installer", func() {
	...
	It("copies the tls cert files", func() {
		g := NewGenerator(workDir, cluster, "", "", "", "", nil, logrus.New(), nil, "", certDir, manifestsAPI, eventsHandler, installerCache, 100, 1*time.Second).(*installerGenerator)
