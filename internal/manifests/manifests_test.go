...
`)
			...
			manifests := []models.Manifest{
				{
					...
					Folder:           validFolder,
					ManifestSource:   string(models.ManifestManifestSourceUserSupplied),
				},
				{
				...
				},
				{
					...
					ManifestSource: string(models.ManifestManifestSourceUnknown),
				},
			}
	...
	Context("V2DeleteClusterManifest", func() {
		It("deletes manifest from default folder", func() {
			clusterID := registerCluster().ID
			mockUpload(1)
			mockS3Client.EXPECT().DoesObjectExist(ctx, getMetadataObjectName(clusterID, defaultFolder, "file-1.yaml", "metadata.json")).Return(true, nil).AnyTimes()
			...
			mockS3Client.EXPECT().DeleteObject(ctx, getMetadataObjectName(clusterID, defaultFolder, "file-1.yaml", "metadata.json")).Return(true, nil)
		...
		It("deletes manifest from a different folder", func() {
			...
			mockS3Client.EXPECT().DeleteObject(ctx, getMetadataObjectName(clusterID, validFolder, "file-1.yaml", "metadata.json")).Return(true, nil)
	...
	Context("UpdateClusterManifest", func() {
		It("moves existing file from one path to another", func() {
			...
			mockS3Client.EXPECT().DoesObjectExist(ctx, getMetadataObjectName(clusterID, defaultFolder, fileNameYaml, "metadata.json")).Return(true, nil).AnyTimes()
			mockS3Client.EXPECT().Upload(ctx, []byte{}, getMetadataObjectName(clusterID, destFolder, destFileName, "metadata.json")).Return(nil).Times(1)
			...
			mockS3Client.EXPECT().DeleteObject(ctx, getMetadataObjectName(clusterID, defaultFolder, fileNameYaml, "metadata.json")).Return(true, nil).Times(1)
		...
		It("updates existing file with new content if content is correct for yaml", func() {
			clusterID := registerCluster().ID
			mockUpload(1)
			mockS3Client.EXPECT().DoesObjectExist(ctx, getMetadataObjectName(clusterID, defaultFolder, fileNameYaml, "metadata.json")).Return(true, nil).Times(1)
			response := manifestsAPI.V2UpdateClusterManifest(ctx, operations.V2UpdateClusterManifestParams{
				ClusterID: *clusterID,
				UpdateManifestParams: &models.UpdateManifestParams{
		...
		It("updates existing file with new content if content is correct for json", func() {
			clusterID := registerCluster().ID
			mockUpload(1)
			mockS3Client.EXPECT().DoesObjectExist(ctx, getMetadataObjectName(clusterID, defaultFolder, fileNameJson, "metadata.json")).Return(true, nil).Times(1)
			response := manifestsAPI.V2UpdateClusterManifest(ctx, operations.V2UpdateClusterManifestParams{
				ClusterID: *clusterID,
				UpdateManifestParams: &models.UpdateManifestParams{
		...
		It("Should use the source file name when the updated file name is omitted ", func() {
			...
			mockS3Client.EXPECT().DoesObjectExist(ctx, getMetadataObjectName(clusterID, defaultFolder, fileNameYaml, "metadata.json")).Return(true, nil).AnyTimes()
			mockS3Client.EXPECT().Upload(ctx, []byte{}, getMetadataObjectName(clusterID, destFolder, fileNameYaml, "metadata.json")).Return(nil).Times(1)
			mockS3Client.EXPECT().DoesObjectExist(ctx, getObjectName(clusterID, defaultFolder, fileNameYaml)).Return(true, nil).Times(1)
