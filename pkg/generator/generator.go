...
type InstallGeneratorDirectoryConfig struct {
	WorkDir string
}

// GetWorkingDirectory determines the working directory path for the generator.
func (c *InstallGeneratorDirectoryConfig) GetWorkingDirectory() string {
	return filepath.Join(c.WorkDir, "install-config-generate")
}

func New(directoryConfig InstallGeneratorDirectoryConfig, log logrus.FieldLogger, s3Client s3wrapper.API, cfg Config,
	...
	return &installGenerator{
		...
		log:              log,
		s3Client:         s3Client,
		workDir:          directoryConfig.GetWorkingDirectory(),
