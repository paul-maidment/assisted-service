...
var _ = Describe("ImportLocalCluster", func() {
	...
	BeforeEach(func() {
		...
		oldReleaseimage = "quay.io/openshift-release-dev/ocp-release@sha256:a266d3d65c433b460cdef7ab5d6531580f5391ffff85d9c475208a56452e4cff"
	})
	...
	var mockClusterImageSetPresent = func(image string) *gomock.Call {
		...
		return clusterImportOperations.EXPECT().
			GetClusterImageSet("local-cluster-image-set").
	...
	var mockFailedToCreateLocalClusterDeployment = func() *gomock.Call {
		...
		agentClusterInstallOwnerRef := metav1.OwnerReference{
			...
			Name:       "local-cluster-cluster-install",
			UID:        agentClusterInstallUID,
		}
	...
	var mockDeleteSecret = func(namespace string, name string) *gomock.Call {
		return clusterImportOperations.EXPECT().
			DeleteSecret(namespace, name).
			Return(nil)
	}

	var mockDeleteClusterImageSet = func() *gomock.Call {
		return clusterImportOperations.EXPECT().
			DeleteClusterImageSet(fmt.Sprintf("%s-image-set", LocalClusterNamespace)).
			Return(nil)
	}

	var mockDeleteClusterDeployment = func() *gomock.Call {
		return clusterImportOperations.EXPECT().
			DeleteClusterDeployment(LocalClusterNamespace, fmt.Sprintf("%s-cluster-deployment", LocalClusterNamespace)).
			Return(nil)	
	}

	var mockDeleteAgentClusterInstall = func() *gomock.Call {
		return clusterImportOperations.EXPECT().
			DeleteAgentClusterInstall(LocalClusterNamespace, fmt.Sprintf("%s-cluster-install", LocalClusterNamespace)).
			Return(nil)	
	}

	It("should not proceed if local cluster namespace does not exist", func() {
		mockClusterVersionFound()
		mockClusterImageSetPresent(releaseImage)
	...
	It("should perform an update if the release image has changed", func() {
		mockClusterVersionFound()
		mockClusterImageSetPresent(oldReleaseimage)
		mockDeleteSecret(LocalClusterNamespace, fmt.Sprintf("%s-admin-kubeconfig", LocalClusterNamespace))
		mockDeleteSecret(LocalClusterNamespace, "pull-secret")
		mockDeleteClusterImageSet()
		mockDeleteClusterDeployment() 
		mockDeleteAgentClusterInstall()
		aciPresent := mockAgentClusterInstallPresent()
		mockNamespaceFound()
		mockNodeKubeConfigsFound()
		mockLocalClusterPullSecretFound()
		mockClusterDNSFound()
		...
		mockCreateAgentClusterInstall()
		createdACIMock := mockAgentClusterInstallPresent()
		gomock.InOrder(aciPresent, createdACIMock)
		mockCreateLocalClusterDeployment()
		result := localClusterImport.ImportLocalCluster()
		Expect(result).To(BeNil())
	})
